...
def getConfig():
    ...
    parser.add_argument('--early_stop_nums', type=int, default=15)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename=f'./log/{args.dataset}/{args.dataset}_seed{args.seed}_depth{args.depth}.log', level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s', filemode='w'
    )
    logging.info(args.__dict__)

    """ load data """
    with open(f"./data/{args.dataset}/train.txt") as f:
        data_list = [x.strip().split(" ") for x in f.readlines()]
    with open(f"./data/{args.dataset}/test.txt") as f:
        test_data_list = [x.strip().split(" ") for x in f.readlines()]
    ...
    for line in train_data_list:
        ...
        train_list.append([smiles, words, label])

    for line in valid_data_list:
        smiles, sequence, label = line
        words = split_sequence(sequence, args.n_gram)
        valid_list.append([smiles, words, label])

    for line in test_data_list:
        smiles, sequence, label = line
        words = split_sequence(sequence, args.n_gram)
        test_list.append([smiles, words, label])
    ...
    train_dataset_load = DataLoader(
        train_dataset, batch_size=args.Batch_size, shuffle=True, num_workers=0, collate_fn=CustomDataSet.collate_fn_ngram
    )
    valid_dataset_load = DataLoader(
        valid_dataset, batch_size=args.Batch_size, shuffle=False, num_workers=0, collate_fn=CustomDataSet.collate_fn_ngram
    )
    test_dataset_load = DataLoader(
        test_dataset, batch_size=args.Batch_size, shuffle=False, num_workers=0, collate_fn=CustomDataSet.collate_fn_ngram
    )
    ...
    for epoch in range(1, args.Epoch + 1):
        ...
        _, _, _, _, valid_Precision, valid_Recall, valid_AUC, valid_PRC = evalueate(model, valid_dataset_load, Loss)
        _, _, _, _, test_Precision, test_Recall, test_AUC, test_PRC = evalueate(model, test_dataset_load, Loss)
        ...
        logging.info(
            ...
            f'valid_Recall: {valid_Recall:.5f} ' +
    ...
    _, _, _, _, Precision, Recall, AUC, PRC = evalueate(model, test_dataset_load, Loss)
